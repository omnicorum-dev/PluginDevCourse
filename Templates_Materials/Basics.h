 ...
 */
inline float db2mag(float dB) { return std::pow(10.0f, dB / 20.0f); }
 ...
 */
inline float mag2db(float mag) { return 20.0f * std::log10(mag); }
 ...
 */
inline float db2pow(float dB) { return std::pow(10.0f, dB / 10.0f); }
 ...
 */
inline float pow2db(float pow) { return 10.0f * std::log10(pow); }
 ...
 * MATLAB equivalent: (exp(gain*x) - exp(-gain*x)) / (exp(gain*x) +
 * exp(-gain*x))
 */
 ...
 * blend: 0.0 = a, 1.0 = b
 */
inline float lerp(float a, float b, float frac) { return a + frac * (b - a); }
 ...
 * MATLAB equivalent: Custom, used in filter design
 */
inline float Q2bw(float centerFreq, float Q) { return centerFreq / Q; }
...
inline float bipolar2uni(float xn) { return 0.5f * xn + 0.5f; }
inline double bipolar2uni(double xn) { return 0.5 * xn + 0.5; }

inline float unipolar2bi(float xn) { return 2.0f * xn - 1.0f; }
inline double unipolar2bi(double xn) { return 2.0 * xn - 1.0; }

inline float doUnipolarModulationFromMin(float modulator, float min,
                                         float max) {
    return modulator * (max - min) + min;
}
inline double doUnipolarModulationFromMin(double modulator, double min,
                                          double max) {
    return modulator * (max - min) + min;
}

inline float doUnipolarModulationFromMax(float modulator, float max,
                                         float min) {
    return max - (1.0f - modulator) * (max - min);
}
inline double doUnipolarModulationFromMax(double modulator, double max,
                                          double min) {
...
inline double doBipolarModulationMinMax(double modulator, double min,
                                        double max) {
    double halfRange = (max - min) * 0.5;
    double midpoint = halfRange + min;
    return modulator * halfRange + midpoint;
}
inline float doBipolarModulationCenterRange(float modulator, float center,
                                            float range) {
    float halfRange = range * 0.5f;
    return modulator * halfRange + center;
}
inline double doBipolarModulationCenterRange(double modulator, double center,
                                             double range) {
