...
APVTS::ParameterLayout Processor::createParameterLayout() {
    ...
    addFloat(layout,
             ...
             Params::thresh_min,
             Params::thresh_max,
             Params::thresh_default,
             Params::thresh_stepSize,
             Params::thresh_skew,
             Params::thresh_suffix);
...
void Processor::prepareToPlay(double sample_rate, int buffer_size) {
    ...
    threshSmooth.prepare(sample_rate, buffer_size, &apvts, Params::thresh_ID);
...
void Processor::processBlock(juce::AudioBuffer<float> &buffer,
    ...
    for (int sample = 0; sample < num_samples; ++sample) {
        ...
        clipper.setThreshold(threshSmooth.getNextValue());
        ...
        for (int channel = 0; channel < num_channels; ++channel) {
            ...
            float yn = clipper.processSample(xn);
